  ...
  <aside class="grid" style="gap:16px">
    <div class="card pad">
      ...
      <div style="margin-top:12px">
        <x-wa-button :href="route('go.wa', data_get($listing,'id',0))" />
      </div>
      @if(isset($listing->views) || isset($listing->wa_clicks))
        <div class="muted" style="font-size:12px;margin-top:8px">
          @isset($listing->views) {{ (int)$listing->views }} tayangan @endisset
          @if(isset($listing->views) && isset($listing->wa_clicks)) • @endif
          @isset($listing->wa_clicks) {{ (int)$listing->wa_clicks }} klik WA @endisset
        </div>
      @endif
    </div>
  </aside>
...
<meta name="description" content="Guru les {{ strtolower($listing->category) }} di {{ $listing->region->name }}. Tarif Rp{{ number_format($listing->price,0,',','.') }}/jam. Hubungi {{ $listing->user->name }} via WhatsApp.">
